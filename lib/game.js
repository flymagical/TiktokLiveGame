...
class GameEngine extends EventEmitter {
  ...
  _startRoundTimer(ms) {
    ...
    this._timer = setTimeout(() => this._revealAnswer(), ms);
  }
  ...
  // End-of-stream "thanks for tonight" card for the top gifter. Pauses the
  // quiz while it's on screen so nobody scores unseen, then carries on.
  showMvp() {
    const durationSec = this.config.mvpCardDurationSec || 30;
    const [mvp, ...runnersUp] = this.scoreboard.getTopGifters(3);
    this.emit("state", { type: "mvp", mvp: mvp || null, runnersUp, durationSec });
