...
class QuestionRotation {
  ...
  // up the whole block, the block starts over. `filter` narrows the bank
  // (e.g. tebak mode only uses questions whose answer can be typed).
  pool(bank, filter = () => true) {
    const inBlock = bank.filter((item) => blockOf(item) === this.state.block && filter(item));
