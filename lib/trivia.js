...
function loadIndonesianQuestions(rotation) {
  const bank = require("./questions-id");
  const pool = rotation ? rotation.pool(bank) : bank;
...
async function fetchQuestions(triviaConfig = {}, rotation = null) {
  ...
  if (!triviaConfig || triviaConfig.source !== "opentdb") {
    return loadIndonesianQuestions(rotation);
  }
